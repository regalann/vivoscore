...
// 🚀 DÜZELTME: Güvenli fetch artık global, böylece heryerden çökmeden erişilebilir
...
app.get('/api/event/:id/lineups', async (req, res) => {
  ...
  try {
    ...
    const formatPlayers = (td) => (td?.players || []).map(p => sanitizeObject({
      id: Number(p.player?.id) || 0, name: p.player?.shortName || p.player?.name || '', number: p.shirtNumber, position: p.position, substitute: p.substitute || false,
      rating: p.statistics?.rating ? parseFloat(p.statistics.rating).toFixed(1) : null, img: `https://api.sofascore.app/api/v1/player/${Number(p.player?.id) || 0}/image`, stats: p.statistics || {}
    }));
    const formatMissing = (td) => (td?.missing || []).map(p => sanitizeObject({
      id: Number(p.player?.id) || 0, name: p.player?.shortName || p.player?.name || '', type: p.type || '', reason: p.reason || ''
    }));
    res.json({ home: { formation: sanitizeString(data.home?.formation || ''), players: formatPlayers(data.home), missing: formatMissing(data.home) }, away: { formation: sanitizeString(data.away?.formation || ''), players: formatPlayers(data.away), missing: formatMissing(data.away) } });
  } catch (err) { res.status(500).json({ error: 'Sunucu hatası' }); }
});
...
// 🚀 BU SATIR ÇOK ÖNEMLİ, KOPYALADIĞINDAN EMİN OL! 🚀
